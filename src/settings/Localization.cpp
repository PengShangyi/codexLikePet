...
QString Localization::text(TextKey key) const
{
    ...
    case TextKey::PetGuideStep2Body: return zh ? QStringLiteral("每行单独生成一条横向长条，每次都附上第 1 步的角色图作为参考。各行与帧数：idle 6、running-right 8、running-left 8、waving 4、jumping 5、failed 8、waiting 6、running 6、review 6。最后是两行朝向，各 8 帧，从 000°（正上方）开始顺时针每 22.5° 一格，分别命名 look-a 与 look-b。把每张图按所属行的名字保存（idle.png、running-right.png……），下一步的“从文件夹填充”就能一次全部认出来。") : QStringLiteral("Generate one horizontal strip per row, attaching the step 1 character as a reference every time. Rows and frame counts: idle 6, running-right 8, running-left 8, waving 4, jumping 5, failed 8, waiting 6, running 6, review 6. Then two look rows of 8 frames each, turning clockwise in 22.5° steps from 000° (up), named look-a and look-b. Save each image under its row's name — idle.png, running-right.png and so on — and the next step's Fill from folder finds all eleven at once.");
    case TextKey::PetGuideStep3Heading: return zh ? QStringLiteral("3. 拼装与校验") : QStringLiteral("3. Assemble and validate");
    case TextKey::PetGuideStep3Body: return zh ? QStringLiteral("没有哪个图像模型能一次就返回像素精确的 1536×2288 图集，所以拼装是单独一步：切帧、抠掉背景色、按网格摆放每一格，再校验结果。下面的拼装工具会替你完成全过程——选好这 11 条长条图，它会统一大小与落地基准线，拼好后直接安装。如果你想自己控制每一个环节，随附的 Hatch Pet 技能提供完整流程；把它复制到 ~/.codex/skills/hatch-pet 即可配合 Codex 使用。Potato 不会替你安装它。") : QStringLiteral("No image model reliably returns a pixel-exact 1536×2288 atlas in one shot, so assembly is its own step: cut the frames, key out the background, place every cell on the grid, and validate the result. The tool below does all of that for you — point it at your eleven strips and it gives them one size and one ground line, assembles the atlas, and installs it. If you would rather drive every stage yourself, the bundled Hatch Pet skill covers the full workflow; copy it to ~/.codex/skills/hatch-pet to use it with Codex. Potato never installs it for you.");
